#!/usr/bin/env python3
"""Generate individual game pages from game metadata."""
...
games_dir = project_root / "src" / "games"
metadata_file = project_root / "src" / "data" / "game-metadata.json"

# Template for game pages
GAME_PAGE_TEMPLATE = """# {name}
...
```js
// Find this specific game (app_id {app_id})
const APP_ID = {app_id};
...
def main() -> None:
    """Generate game pages from metadata."""
    ...
    for game in games:
        app_id = game["app_id"]
        name = game["name"]
        page_file = games_dir / f"{app_id}.md"

        content = GAME_PAGE_TEMPLATE.format(name=name, app_id=app_id)

        with open(page_file, "w") as f:
            f.write(content)

        print(f"✅ Generated page for {name} ({app_id}.md)")

    print(f"\n✨ Done! Generated {len(games)} game pages in {games_dir}")
