...
class TestSteamCollector:
    ...
    def test_get_player_count_success(self) -> None:
        ...
        with patch("requests.get") as mock_get:
            ...
            mock_response.json.return_value = {"response": {"player_count": 123456, "result": 1}}
    ...
    def test_get_player_count_with_game_name(self) -> None:
        """Test player count returns game metadata."""
        collector = SteamCollector()

        with patch("requests.get") as mock_get:
            mock_response = Mock()
            mock_response.status_code = 200
            mock_response.json.return_value = {"response": {"player_count": 50000, "result": 1}}
            mock_get.return_value = mock_response

            data = collector.get_game_data(570)  # Dota 2

            assert data["app_id"] == 570
            assert data["player_count"] == 50000
    ...
    def test_get_player_count_missing_data(self) -> None:
        """Test handling of missing player_count in response."""
        collector = SteamCollector()
    ...
    def test_retry_on_failure(self) -> None:
        ...
        with patch("requests.get") as mock_get:
            ...
            mock_get.side_effect = [
                ...
                Mock(
                    status_code=200,
                    json=lambda: {"response": {"player_count": 99999, "result": 1}},
                ),
            ]
